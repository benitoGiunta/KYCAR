...
## H. Arbitrages après la recette 2.9b (`reports/ACCEPTANCE.md`, G8 FRANCHIE, ACC-01…15)

| # | Sujet | Décision |
|---|---|---|
| D8-41 | **ACC-01 (MAJEUR)** : en mode 2, le filtre Carrosserie (`body=`) n'est ni appliqué ni déclaré — effectif non filtré sous un jeton actif, sans le bandeau D8-20 (`EX-SCR-221`, D-03 « jamais ignoré en silence », `EX-NAV-18`). Cause localisée par l'acceptance : `enterMode2` (`data-controller.ts`) ne reporte pas la composante T dans `unappliedFilterIds` | **Corrigé avant clôture** (lot `fix-app-3`, Opus/high, arbre principal) : `unappliedFilterIds` porte le filtre dès l'entrée en mode 2 → le bandeau existant s'affiche ; sonde D8 rouge d'abord ; test E2E sur `/marche/54-opel/1918-corsa?body=3` (bandeau présent, effectif inchangé et **déclaré**). Aucune valeur inventée : le chiffre reste celui de la cellule, la mention dit pourquoi. |
| D8-42 | **ACC-05** (« 0 modèles » pendant ~0,5–0,7 s avant le cardinal réel, contraire à D8-02 « — tant que la donnée manque ») et **ACC-15** (« 1 marques » sans singulier) — barre de synthèse (`src/screens/market/SummaryBar.tsx`, `view-model.ts`) | **Corrigés dans le même lot** (bornés, une valeur affichée fausse ou transitoirement fausse) : `—` tant que `modelCount` n'est pas chargé ; accord singulier/pluriel des trois cardinaux. Sondes D6 rouges d'abord. |
| D8-43 | **ACC-02, ACC-03** (MAJEUR, présentation : bandeau collant inerte et hauteur repliée > viewport ; régime compact de l'écran B) et **ACC-04, 06–14** (MINEUR) | **Dette de présentation consignée**, non corrigée dans cette session : ce sont des exigences « mesures au rendu » que 2.7 avait renvoyées à 2.9b et qui n'ont jamais été implémentées au pixel près ; aucune n'invente une valeur, aucun parcours ne se rompt. Décision de les traiter (phase 2.10 « finition visuelle ») **au commanditaire**, avec `ACCEPTANCE.md` §8 comme cahier des charges. ACC-06 (`sel` 2D → écran D) est signalé comme le premier à reprendre si une 2.10 est ouverte. |
| D8-44 | Re-recette après le lot fix-app-3 | `acceptance` relancé par message (rev 2) : suite E2E complète 3 projets + rejeu ciblé d'ACC-01/05/15 ; `ACCEPTANCE.md` mis à jour (§8 statuts, §9 avis) ; `results.json` commité avec la rev 2 (D8-33). |
