...
Dernière mise à jour : 2026-09-08 — **Chantier 1 CLOS. Chantier 2 : 2.0→2.6 VALIDÉ (branche `claude/kycar-project-ffcplk`). Reste 2.7 (vérification finale), non lancée — attend feu vert commanditaire**
...
| E2 | ~~Aucun modèle Fable, sur aucune tâche~~ **LEVÉE le 2026-09-08** par le commanditaire : tout modèle est autorisé ; le choix modèle/effort par rôle est documenté dans `CLAUDE.md` § Organisation en agents et `.claude/agents/` | consigne du commanditaire, levée |
...
| 2.4 Développement D1–D9 | `dev-D*` | voir plan 2 | code (branche `phase-2.4-build`) | ✅ **VALIDÉ — phase 2.4 COMPLÈTE (9/9 lots).** 536 tests verts, build 0/0, lint vert, bundle 68/300 Ko gzip, app dev vérifiée en direct (7 vues), 2 parcours cibles + repli provider testés. Orchestration : D1→D2 en séquence directe (fondation) ; D3∥D4∥D5∥D9 en worktrees isolés ; D6∥D7 après D4+D5 ; D8 intégration en place. **D1 ✅ VALIDÉ** (build/lint/test/size verts, worker PING/PONG OK, outillage installé une fois pour toutes). **D2 ✅ VALIDÉ** (45 tests ; 13 entités, colonnaire+sentinelles, validation R3, 8 invariants, codec selectionHash, chargeur référentiels, DataProvider intégré ; SHA-256 maison synchrone ; 4 divergences remontées → O13–O16). Vague parallèle **D3∥D4∥D5∥D9** en worktrees. **D9 ✅ FUSIONNÉ** (TweedehandsDataProvider : mode 1 réel via `__NEXT_DATA__` autorisé, `servesMode2()=false`, R3 prouvé à l'ingestion, mapping vocab 2dehands→KYCAR, **zéro appel réseau** — 47 tests, 92 au total). **D3/D4/D5 morts simultanément à un reset de session** (mode de défaillance connu), travail préservé et commité par le coordinateur. **D3 ✅ FUSIONNÉ** (SyntheticDataProvider 100k, outliers injectés + vérité terrain, déterministe ; 5 fixes de typage de test finis par le coordinateur). **D4 (moteur) FUSIONNÉ** — code complet qui compile, mais **vérification à compléter** → agent `D4-verif` (Opus) écrit banc perf 200ms/100k + invariants + détection M1/M2. **D5 (cœur) FUSIONNÉ** — codec URL/registre 77 filtres/T-R/corrections, mais **UI+routeur+débounce à compléter** → agent `D5-ui` (Sonnet). **D5 ✅ CLOS** (D5-ui fusionné : routeur 6 routes, contrôleur débounce/historique, bandeau 77 filtres, écran G ; 283 tests). Correctifs coordinateur : lint prng.ts (D3) + eslint ignore `.claude/`. **D4 ✅ CLOS** (D4-verif fusionné : bug réel corrigé agrégation 25 s→0,18 s ; invariants I1–I8, quantiles exacts, facettes 31 ms, élagage 5556×, outliers M1 95,8 %/M2 100 % en cellules éligibles ; écart EX-NFR-5 sur pire cas non routé → O17). Bancs de perf séparés du `npm test` (→ `test:perf`). **Vague cœur complète : D3/D4/D5/D9 tous fusionnés, 299 tests par défaut verts.** Dernière vague : **D6 (écran A) ∥ D7 (écran B/nuage/liste)**. **D7 ✅ FUSIONNÉ** (histogrammes SVG, nuage G4 Canvas 2D échantillonnage déterministe/brossage, écran D + CSV sans R3 ; perf nuage p95 5,99 ms, 100 % fenêtres ≥30 img/s ; 358 tests). Fiabilisation : `testTimeout` par défaut → 30 s (flake dataset 100k sous charge). **D6 ✅ FUSIONNÉ** (écran A : cartes-marques/zones-modèles, fourchette « centrale » étiquetée, tri, 6 états, CSV sans R3, montage du sélecteur G de D5). **Vague écrans complète. Base combinée : 502 tests verts.** **D8 TERMINÉ** après reprise : bloquant types Node levé (`@types/node` devDep + `node` dans les types tsconfig) ; WIP récupéré depuis `wip/d8-integration` ; coquille+routage (`app.tsx`/`main.tsx`), écrans C/E/F + `/mentions`, persistance (`localStorage` CRUD + IndexedDB cache snapshot, concurrence inter-onglets, migration), états dégradés, 34 tests dont les 2 parcours + repli. **Phase 2.4 close.** Suite (2.5 revue, 2.6 remédiation traitant O13–O17, 2.7 vérif) NON lancée — hors périmètre |
| 2.5 Revue de développement | `rev-D1…D9` + `rev-patho` (parallèle, 10 agents : Fable D4/D8, Opus D2/D3/D5/D7/D9/patho, Sonnet D1/D6) → `rev-consolidate` (Opus) | voir `CLAUDE.md` § Organisation | `tests/review/` (783 sondes), `reports/review/D*.md`, `reports/DEV-REVIEW.md` | ✅ **VALIDÉ** le 2026-09-08 — 190 constats sources → 160 consolidés (18 BLOQUANT, 86 MAJEUR, 56 MINEUR), 116 critères jugés (83 ATTEINT), 205 sondes rouges = constats exécutables ; S1–S4 de PLAN-2 §2.5 atteints. 5 défauts systémiques : drapeaux d'ingestion jamais posés, filtres ignorés en silence, contrat d'URL D5≠D7, interfaces gelées trop étroites (`makeId` Int16, 17ᵉ drapeau), M1/M2 gardés par convention. O17 câblage prouvé sain (D8) |
| 2.6 Remédiation | coordinateur = fix-lead (Fable) → `fix-foundation` (Opus, étape 0 séquentielle) → F1 parallèle en worktrees : `fix-engine`/`fix-providers` (Opus), `fix-state`/`fix-screens`/`fix-docs` (Sonnet) → F2 : `fix-app` + `fix-residual` (Opus) → `fix-verify` (Opus) | voir `CLAUDE.md` | `reports/remediation/FIX-LEAD-DECISIONS.md` (D-01…D-51), `reports/remediation/fix-*.md`, `reports/REMEDIATION.md` | ✅ **VALIDÉ** le 2026-09-08 — interfaces gelées amendées (`makeId` Int32, `ingestFlags` Uint32 + table bit↔code, `unsupportedFilterIds`), REQUIREMENTS v1.1 (32 amendements tracés), 148 constats corrigés, 11 dettes consignées (dont 3 MAJEUR motivés : DR-034, DR-082 TVA, DR-104), sondes 205 rouges → 0 non couverte (8 `it.fails` annotées), suite `npm test` = 615 + 798 verts, perf p95 non élagué 657 → < 200 ms, index 82 → 2,5 Mo, premier affichage 1 127 → ~150 ms, parcours 2 « Opel Corsa 2017 » exerçable (1 352 annonces). Porte G5 : voir `REMEDIATION.md` §6 |
| 2.7 Vérification finale | `final-check` | **Fable / max** (E2 levée) | `reports/FINAL-VERIFICATION.md` | À FAIRE — sur feu vert commanditaire |
...
| O13 (2.4/D2) | **`KYCAR_INGEST_FLAG` : 14 vs 17 codes.** §A.1 annonce 14 et le champ #82 borne `0..14`, mais `EX-DATA-45` nomme 17 codes un à un. Or `ListingColumnBatch.ingestFlags` (interface gelée 2.3) est un `Uint16Array` = 16 bits/ligne : 17 drapeaux n'y tiennent pas | Divergence entre l'interface gelée et l'annexe A | **RÉSOLU en 2.6 (`D-01`)** : `ingestFlags` passe à `Uint32Array` (32 bits/ligne). Encodage positionnel conservé, mais découplé de la largeur de colonne par une table explicite `INGEST_FLAG_BIT` (bit ↔ code), exportée par `src/types`. §A.1 porte désormais 17 codes, 15 bits de réserve |
| O14 (2.4/D2) | **Fichiers région BE absents du dépôt** : `regions-be.json` / `postal-regions-be.json` cités par le dictionnaire n'existent pas. Table NUTS-2 BE encodée en code (`vocabularies.ts`), dette `[EXTRAPOLÉ]` `EX-DATA-53` | Le mapping code postal → NUTS-2 repose sur une table extrapolée | **Mis en DETTE en 2.6, non soldé (DR-112)** : produire le fichier exige une source externe officielle (Statbel/bpost) que la contrainte E5 interdit de collecter en session. La couverture actuelle reste exhaustive (9 000 codes postaux, plages disjointes et contiguës) et la dette est visible à l'exécution (`[EXTRAPOLÉ]`). Reporté en 2.7 |
| O15 (2.4/D2) | **`Model.bodyTypes` vide** : `taxonomy.json` ne porte pas `bodyTypes` par modèle ; l'index taxonomique par carrosserie (`EX-DATA-115bis`), condition de la classe R du filtre Carrosserie, sera vide tant que la donnée n'est pas fournie | Filtre Carrosserie en classe R inopérant à l'écran A (D6) tant que la donnée manque | **Structure RÉSOLUE en 2.6, donnée toujours absente** : l'index taxonomique par carrosserie est désormais exposé (vide) avec son drapeau de disponibilité (sonde `R-D2-17` verte, contrôlée par fix-verify) ; `bodyTypes` reste vide sur les 4 955 modèles, la donnée reste à fournir contre le référentiel source (point d'instruction 2.7) |
| O17 (2.4/D4) | **`EX-NFR-5` (recalcul ≤ 200 ms p95) : tenue sur les chemins de production, PAS sur un pire cas non routé.** Banc D4-verif à N=100k (dataset D3) : sélection **élaguée** (mode 2, chemin interactif réel) **37 ms p95** ✅ ; facettes différées **31 ms** ✅ ; sélection vide **précalculée** (0 balayage, EX-DATA-109) ✅. Mais recalcul **complet non filtré avec M1/M2** = **p95 ~720–985 ms** ❌, dominé par la régression robuste M2 à deux passes exécutée au niveau sélection sur 100k. Or l'architecture §3.2 route ce cas : mode 1 vide = précalculé, M1/M2 = mode 2 uniquement, donc **toujours élagué** à `m`. | Le moteur tient le budget sur tous les chemins **conçus** ; l'écart n'existe que sur un chemin que l'app n'est pas censée déclencher (M2 sur sélection non élaguée). | **Câblage applicatif prouvé sain par D8** (9 sondes vertes : aucun chemin résiduel — URL directe, `modelId = 0`, retour arrière, changement de modèle, « Rafraîchir » — ne déclenche `recalculate` hors élagage). **La garde côté API moteur reste en cours de correction par fix-engine** (`EngineSelection.scope` facultatif, `recalculate` exécutant `detectOutliers` sans garde, DR-032) : non vérifiable depuis ce lot, à confirmer par le rapport de fix-engine |
| O16 (2.4/D2) | **13 vs 14 entités** : `EX-DATA-105`/ARCHITECTURE §2.1 disent « 13 » mais en nomment 14. D2 a typé les 14 concepts nommés | Cosmétique, aucun impact fonctionnel | **RÉSOLU en 2.6** : `EX-DATA-105`, `draft-data-dictionary.md` §C.5 et `ARCHITECTURE.md` §2.1 portent désormais tous « quatorze entités » |
