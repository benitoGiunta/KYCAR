...
Dernière mise à jour : 2026-09-08 — **Chantier 1 CLOS. Chantier 2 : 2.0→2.4 VALIDÉ ; 2.5 (revue) et 2.6 (remédiation) LANCÉES sur feu vert du commanditaire (branche `claude/kycar-project-ffcplk`, basée sur `phase-2.4-build`)**
...
| 2.5 Revue de développement | `rev-D1…D9` + `rev-patho` (parallèle) → `rev-consolidate` | voir `CLAUDE.md` § Organisation (Fable D4/D8, Opus D2/D3/D5/D7/D9/patho, Sonnet D1/D6) | `tests/review/`, `reports/review/D*.md`, `reports/DEV-REVIEW.md` | EN COURS — lancée le 2026-09-08 |
| 2.6 Remédiation | coordinateur (fix-lead) + `fix-engine/app` (Opus) + `fix-providers/state/screens/docs` (Sonnet) → `fix-verify` (Opus) | voir `CLAUDE.md` | `reports/remediation/*.md`, `reports/REMEDIATION.md` | À FAIRE — démarre à la clôture de 2.5 |
