...
| O15 (2.4/D2) | **`Model.bodyTypes` vide** : `taxonomy.json` ne porte pas `bodyTypes` par modèle ; l'index taxonomique par carrosserie (`EX-DATA-115bis`), condition de la classe R du filtre Carrosserie, sera vide tant que la donnée n'est pas fournie | Filtre Carrosserie en classe R inopérant à l'écran A (D6) tant que la donnée manque | **État factuel, non résolu en 2.6** : `bodyTypes` reste vide sur les 4 955 modèles, l'index taxonomique par carrosserie reste absent (ni index vide, ni drapeau de disponibilité). La donnée reste à fournir contre le référentiel source ; aucune correction de code n'a été apportée en phase 2.6 |
