...
import { buildAggregateCsv } from '../../../src/screens/market/csv';
...
describe('EX-DATA-123bis — colonnes normatives de l’export « Agrégats mode 1 »', () => {
  ...
  it("R-D6-03 — l'en-tête produit ne contient PAS les colonnes normatives d'EX-DATA-123bis (median, p5, p95, n_prix, n_annee, n_km absentes)", () => {
    const csv = sampleCsv();
    const header = csv.slice(1).split('\r\n')[0]; // slice(1) retire le BOM
    // Assertion normative (échoue tant que non corrigé — c'est le constat) :
  ...
  it("R-D6-05 — le nom de fichier ne suit pas le gabarit normatif `kycar_<perimetre>_<snapshotId>_<AAAAMMJJ>.csv`", () => {
    const src = readFileSync(new URL('../../../src/screens/market/MarketScreen.tsx', import.meta.url), 'utf8');
    const match = /anchor\.download\s*=\s*'([^']+)'/.exec(src);
    expect(match?.[1]).toMatch(/^kycar_[a-z0-9-]+_[a-zA-Z0-9-]+_\d{8}\.csv$/);
  });
});
